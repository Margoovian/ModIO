...
class SawtoothGeneratorNode : public ImFlow::BaseNode
{

public:
    SawtoothGeneratorNode()
    {

        m_Generator = std::make_unique<ModIO::Generators::SawtoothGenerator>();

        m_Generator->mFrequency = m_Frequency;
        m_Generator->mAmplitude = m_Volume * 0.01f;

        setTitle("Sawtooth Wave Generator");
...
private:
    ModIO::TD::UniqueSawtoothGenerator m_Generator;
